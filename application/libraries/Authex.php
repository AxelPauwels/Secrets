...
class Authex {
    ...
    function getUserId_byEmail($email) {
        // geef user-id als gebruiker aangemeld is
        $CI = &get_instance();
        return $CI->user_model->getUserId_byEmail($email);
    }
...
// EPISODES **************************************************************************
    ...
    function getAlleEpisodesSeizoen() {
        $CI = &get_instance();
        $CI->load->model('episodesSeizoen_model');
        $episodesSeizoen = $CI->episodesSeizoen_model->getAlleEpisodesSeizoen();

        foreach ($episodesSeizoen as $collectie) {
            $collectie->aantalSeizoenen = $CI->episodesSeizoen_model->getAantalSeizoenenPerCollectie($collectie->collectie);
        }
